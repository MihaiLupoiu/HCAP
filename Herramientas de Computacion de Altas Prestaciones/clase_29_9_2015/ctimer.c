...
int ctimer_(double *elapsed, double *ucpu, double *scpu) {
  ...
  //printf("usegs = %lf\n",usegs);
  if (nofirstcall)  {
    //printf("2. nofirstcall = %lf\n",nofirstcall);
    *elapsed = usegs - nofirstcall;
    nofirstcall = 0.0;
  } else {
    //printf("1. nofirstcall = %lf\n",nofirstcall);
    *elapsed = 0;
    nofirstcall = usegs;
  }

  *elapsed = *elapsed/1E6;
  *ucpu = (double)sistema.tms_utime/(double)CLOCKS_PER_SEC*1E3;
  *scpu = (double)sistema.tms_stime/(double)CLOCKS_PER_SEC*1E3;
