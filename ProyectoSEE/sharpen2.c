...
int main(int argc, char *argv[])
{
    ...
    FLOAT tempR,tempG,tempB, clkRate;
 ...
 //   printf(" %7.1f Mhz\n", clkRate);
    ...
    __m128 mmx0_vR, mmx1_vG, mmx2_vB;
    __m128 mmx3_vR_t, mmx4_vG_t, mmx5_vB_t;
    ...
    for(i=1; i<239; i++)
    {
        ...
        for(j=1; j<319; j++)
        {
            ...
            mmx3_vR_t = _mm_mul_ps(mmx6_vPSF, mmx0_vR);
            mmx4_vG_t = _mm_mul_ps(mmx6_vPSF, mmx1_vG);
            mmx5_vB_t = _mm_mul_ps(mmx6_vPSF, mmx2_vB);
            ...
            mmx0_vR = _mm_mul_ps(mmx6_vPSF, mmx0_vR);
            mmx1_vG = _mm_mul_ps(mmx6_vPSF, mmx1_vG);
            mmx2_vB = _mm_mul_ps(mmx6_vPSF, mmx2_vB);

            mmx3_vR_t = _mm_add_ps(mmx3_vR_t, mmx0_vR );
            mmx4_vG_t = _mm_add_ps(mmx4_vG_t,mmx1_vG);
            ...
            ////////////////////////////////////////////////////////////////////
            mmx3_vR_t = _mm_hadd_ps(mmx3_vR_t, mmx3_vR_t);
            mmx3_vR_t = _mm_hadd_ps(mmx3_vR_t, mmx3_vR_t);

            mmx4_vG_t = _mm_hadd_ps(mmx4_vG_t, mmx4_vG_t);
            mmx4_vG_t = _mm_hadd_ps(mmx4_vG_t, mmx4_vG_t);

            mmx5_vB_t = _mm_hadd_ps(mmx5_vB_t, mmx5_vB_t);
            mmx5_vB_t = _mm_hadd_ps(mmx5_vB_t, mmx5_vB_t);


            _mm_store_ss(&tempR, mmx3_vR_t);
            _mm_store_ss(&tempG, mmx4_vG_t);
            _mm_store_ss(&tempB, mmx5_vB_t);


            if(tempR<0.0) tempR=0.0;
            if(tempG<0.0) tempG=0.0;
            if(tempB<0.0) tempB=0.0;


            if(tempR>255.0) tempR=255.0;
            if(tempG>255.0) tempG=255.0;
            if(tempB>255.0) tempB=255.0;

            convR[(i*320)+j]=(UINT8)tempR;
            convG[(i*320)+j]=(UINT8)tempG;
            convB[(i*320)+j]=(UINT8)tempB;
